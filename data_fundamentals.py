...
def _alphavantage_rate_limited(data: dict | None, error: Exception | None = None) -> bool:
    if error is not None:
        if isinstance(error, requests.HTTPError) and error.response is not None:
            if error.response.status_code == 429:
                return True
        message = str(error).lower()
        if "rate limit" in message or "call frequency" in message:
            return True
    ...
    for key in ("Note", "Information", "Error Message"):
        ...
        if "rate limit" in text or "call frequency" in text or "sparing" in text:
            return True
    return False
...
def _alphavantage_overview(ticker: str) -> dict:
    ...
    for attempt in range(2):
        ...
        try:
            ...
            if response_data.get("Symbol") is None:
                msg = (
                    response_data.get("Note")
                    or response_data.get("Information")
                    or response_data.get("Error Message")
                )
                if _alphavantage_rate_limited(response_data):
                    raise ValueError(msg or "Alpha Vantage rate limit")
                raise ValueError(msg or f"Alpha Vantage returned no overview for {ticker}")
            return response_data
        except Exception as e:
            last_error = e
            if attempt == 0 and _alphavantage_rate_limited(response_data, e):
                print(
                    f"[warn] Alpha Vantage rate limit for {ticker}, "
                    "retrying once in 3s before yfinance fallback"
                )
                time.sleep(3)
                continue
            raise
